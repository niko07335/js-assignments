...
function removeLeadingAndTrailingWhitespaces(value) {
    let res = value;
    while (res[0] === ' ' || res[0] === '\t') {res = res.slice(1)};
    while (res[res.length-1] === ' ' || res[res.length-1] === '\t'){ res = res.slice(0, -1)};
    return res;
}
...
function removeFirstOccurrences(str, value) {
    ...
    let i = value.length;
    let res = str.substr(0, x)+str.substr(x+i);
    return res;
}
...
function encodeToRot13(str) {
    let z = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let x = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let i = 0;
    let res = str.split('');
    for  (i = 0; i < res.length; i +=1) {
        if (z.indexOf(res[i]) !== -1) {res[i] = x[z.indexOf(res[i])]}
    }
    return res.join('');
}
...
function isString(value) {
    let res = (typeof(value) === 'string' || value instanceof String) 
    return res;
